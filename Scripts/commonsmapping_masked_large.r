...
#zonal_stats_seg <- write.table(zonal_stats_seg, "Zonal_stats/zonal_stats_seg_20170801_MASKED_LARGE.txt", sep="\t")
zonal_stats_seg <- read.table("Zonal_stats/zonal_stats_seg_20170801_MASKED_LARGE.txt", sep="\t", header=T)
...
write.table(zonal_stats_seg, "zonal_stats/zonal_stats_seg_20170801_MASKED_LARGE.txt", sep="\t")
...
{
  zonal_stats_seg <- read.table("zonal_stats/zonal_stats_seg_20170801_MASKED_LARGE.txt", sep="\t", header=T, as.is=T)
}
segmentation.raster <-raster("Segmentation/Segmentation_LARGE_Rasterised.tif")
#segmentation.raster <-raster("Segmentation/Segmentation_Rasterised.tif")
#segmentation.raster <-raster("Segmentation/Test_Segmentation_Rasterised.tif")
...
write.table(training.data, "training_data/training_data_20170801_MASKED_LARGE.txt", sep="\t")
write.table(training.data.test, "training_data/training_data_test_20170801_MASKED_LARGE.txt", sep="\t")
...
training.data <- read.table("training_data/training_data_20170801_MASKED_LARGE.txt", sep="\t", header=T)
training.data.test <- read.table("training_data/training_data_test_20170801_MASKED_LARGE.txt", sep="\t", header=T)

training.data$PRIHAB <- as.factor(as.character(training.data$PRIHAB))

# Predict habitats using random forest
#Run for only the top 80 most important variables
M.rf.detailed.all <- randomForest(PRIHAB ~ ., data=training.data[c(2,4:ncol(training.data))], na.action=na.omit)
i <- colnames(training.data) %in% c(rownames(M.rf.detailed.all$importance)[order(M.rf.detailed.all$importance, decreasing=T)][1:20],"PRIHAB")
...
writeOGR(segmentation.p,
         "Outputs/CommonsMapping_20170801_MASKED_LARGE.shp",
         "CommonsMapping_20170801_MASKED_LARGE",
