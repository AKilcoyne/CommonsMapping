...
#
# MASKED NORMAL SEGMENTS MINUS SPOT
...
training.data.habitat.shp <- readOGR("Training_Data/FieldData_BNG_edits_bsh_20170829_normalsegments.shp", "FieldData_BNG_edits_bsh_20170829_normalsegments")
...
zonal_stats_seg <- read.table("zonal_stats/zonal_stats_seg_20170829_MASKED_NORMAL.txt", sep="\t", header=T)

# Remove SPOT from zonal_stats_seg and write to new table
#names(zonal_stats_seg[56:67])
zonal_stats_seg[56:67] <- list(NULL)
write.table(zonal_stats_seg, "zonal_stats/zonal_stats_seg_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t")
...
if (!"area_ratio1" %in% names(zonal_stats_seg))
{
  ...
  zonal_stats_seg$area_ratio1 <- with(zonal_stats_seg, Area_m2/Permi_m2)
  zonal_stats_seg$area_ratio2 <- with(zonal_stats_seg, Permi_m2/sqrt(Area_m2))
}
...
nmax <- 40 #### Number of training points per class

if (!exists("zonal_stats_seg"))
{
  zonal_stats_seg <- read.table("zonal_stats/zonal_stats_seg_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t", header=T, as.is=T)
}
#segmentation.raster <-raster("Segmentation/Segmentation_LARGE_Rasterised.tif")
segmentation.raster <-raster("Segmentation/Segmentation_Rasterised.tif")
#segmentation.raster <-raster("Segmentation/Test_Segmentation_Rasterised.tif")
...
training.data.all <- subset(training.data.all, DOMINANCE<=3)
...
for(c in unique(training.data.all$PRIHAB))
{
  ...
  prb <- ifelse(training.data.sub$DOMINANCE == 1,0.95, ifelse(training.data.sub$DOMINANCE == 2, 0.85, 0.20))
...
write.table(training.data, "training_data/training_data_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t")
write.table(training.data.test, "training_data/training_data_test_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t")
...
training.data <- read.table("training_data/training_data_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t", header=T)
training.data.test <- read.table("training_data/training_data_test_20170829_MASKED_NORMAL_NOSPOT.txt", sep="\t", header=T)
...
writeOGR(segmentation.p,
         "Outputs/CommonsMapping_20170829_MASKED_NORMAL_NOSPOT.shp",
         "CommonsMapping_20170829_MASKED_NORMAL_NOSPOT",
