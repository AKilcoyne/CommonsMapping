...
#training.data.habitat.shp <- readOGR("Training_Data/Catchment/Living_Maps_FEP_Data_Catchment_Training_Points_170320.shp", "Living_Maps_FEP_Data_Catchment_Training_Points_170320")

training.data.os.shp <- readOGR ("OS/Cumbria_Final/OS_VectorMapTraining_final_170313.shp", "OS_VectorMapTraining_final_170313")

#training.data.habitat.shp <- readOGR("Training_Data/Additional_Data/LCM/LCM2007_2000_test.shp", "LCM2007_2000_test")

training.data.habitat.shp <- readOGR("Training_Data/Additional_Data/PHI/Living_Maps_FEP_Data_Catchment_Training_Points_170320_plusPHI_20170626.shp", "Living_Maps_FEP_Data_Catchment_Training_Points_170320_plusPHI_20170626")
...
write.table(zonal_stats_seg, "zonal_stats/Catchment/zonal_stats_seg_20170607.txt", sep="\t")
...
{
   zonal_stats_seg <- read.table("zonal_stats/Catchment/zonal_stats_seg_20170607.txt", sep="\t", header=T, as.is=T)
}
segmentation.raster <-raster("Segmentation/Living_Maps_Segmentation_Cumbria.tif")
...
#Calculate easting and northing for the concatenated data
training.data.shp$easting <- coordinates(training.data.shp)[,1]
training.data.shp$northing <- coordin
#####zonal_stats_seg <- merge(zonal_stats_seg, training.data[,c("easting", "northing")], by="seg.id")
#still doesn't work

# Select the required columns
training.data <- trainingates(training.data.shp)[,2]

#test
write.table(zonal_stats_seg, "zonal_stats/Catchment/zonal_stats_seg_20170627_TEST.txt", sep="\t")
zonal_stats_seg <- read.table("zonal_stats/Catchment/zonal_stats_seg_20170627_TEST.txt", sep="\t", header=T, as.is=T)
...
training.data <- merge(data.frame(ID=1:nrow(training.data.shp), seg.id=training.data.ids), zonal_stats_seg, by.x="seg.id", by.y="ID")
.data[c(2:7,11:ncol(training.data))]
...
if (!exists("zonal_stats_seg"))
{
  zonal_stats_seg <- read.table("zonal_stats/zonal_stats_seg_20170607.txt", sep="\t", header=T, as.is=T)
}
...
i <- colnames(training.data) %in% c(rownames(M.rf.detailed.all$importance)[order(M.rf.detailed.all$importance, decreasing=T)][1:42],"Feature_De")
...
# Predict classes for all polygons | Error in eval(expr, envir, enclos) : object 'easting' not found
...
writeOGR(segmentation.p,
         "Outputs/FINAL/Living_Maps_Catchment_RF_Detailed_20170626.shp",
         "Living_Maps_Catchment_RF_Detailed_20170626",
